...
now = datetime.now()
logging.basicConfig(filename=f"Logs/NHL.log", filemode='w', level=logging.INFO, format='%(name)s - %(levelname)s - %(message)s')
...
NHL_GAME_HEADERS = ["Game", "DK Home SGP ML", "DK Home non sgp ML", "Difference"]
NHL_GAME_DATA = []
    ...
    try:
        ...
        try:
            ...
            for header in header_elements:
                ...
                if header_name == "Moneyline":
                    button_elements = header.find_elements(By.CLASS_NAME, "rj-market__button--yourbet")
                    for element in button_elements:
                        key = element.find_element(By.CLASS_NAME, "rj-market__button-yourbet-title").text
                        val = element.find_element(By.CLASS_NAME, "rj-market__button-yourbet-odds").text.replace("−", "-")
                        sgp[key] = val
...
def write_excel_sheet():
    ...
    df_nhl = pd.DataFrame(NHL_GAME_DATA, columns=NHL_GAME_HEADERS)
    ...
    logging.info(f"There are: {len(NHL_GAME_DATA)} Game ML Data ")
    print(f"There are: {len(NHL_GAME_DATA)} Game ML Data")
...
def NHL_CALL():
    ...
    try:
        logging.info("NHL Call")
        try:
            nhl_url = "https://sportsbook.draftkings.com/leagues/hockey/nhl"
            links = get_match_links(nhl_url, driver)
            # links = ["https://sportsbook.draftkings.com/event/tb-lightning-%40-was-capitals/27889744?sgpmode=true"]
